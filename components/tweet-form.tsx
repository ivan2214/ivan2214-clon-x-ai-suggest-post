...
export default function TweetForm({currentUser}: TweetFormProps) {
  ...
  const extraerTweets = (texto: string) => {
    ...
    const tweets = partes.filter((parte) => parte.trim() !== "").map((parte) => parte.trim())
  ...
  const handleSuggestionTweet = async () => {
    ...
    const tweets = extraerTweets(result.message)
    const mediaUrl = extraerUrl(result.message)
          ...
          <Form {...form}>
            <form className="flex w-full flex-col gap-y-4" onSubmit={form.handleSubmit(onSubmit)}>
              <div className="flex w-full flex-col gap-y-5">
                <div>
                  <Button className="rounded-xl text-xs text-sky-600" size="sm" variant="outline">
                    Todos
                    <svg
                  ...
                  render={({field}) => (
                    <FormItem>
                      <FormControl>
                        <Textarea
                          {...field}
                          className="resize-none border-none text-xl focus-visible:ring-0"
                          disabled={isPending}
              ...
              {tweetSuggestion.content && showSuggestions && tweetSuggestion.content?.length > 0 ? (
                  ...
                  {tweetSuggestion.content?.map((t, index) => {
                    ...
                    return (
                      <li
                        key={t}
                        className={cn(
                          "cursor-pointer p-2 font-light text-gray-400 hover:rounded-md hover:bg-sky-600/60 hover:text-white",
                          index !== content.length - 1 && "border-b pb-2",
                        )}
                      >
                        <div onClick={() => handleSelectTweet(index)}>
                          {t}
                          {mediaUrl ? (
                            <img alt={t} className="h-10 w-10 object-cover" src={mediaUrl.url} />
                          ) : null}
                        </div>
                      </li>
                    )
              ...
              <section className="flex w-full items-center gap-x-3">
                <Button
                  className="bg-sky-600 font-bold text-white hover:bg-sky-700"
                  type="button"
                  onClick={handleSuggestionTweet}
                >
                  Sugerir tweet con IA
                </Button>
                {tweetSuggestion.content && tweetSuggestion.content.length > 0 ? (
                  <Button
                    className="bg-sky-600 font-bold text-white hover:bg-sky-700"
                    type="button"
              ...
              <span className="text-xs text-sky-600">🌐 Cualquier persona puede responder</span>
              <section className="flex w-full items-center justify-between border-t pt-2">
                ...
                <Button className="border-2 bg-sky-600" disabled={isPending} type="submit">
                  Postear
                </Button>
              </section>
            </form>
